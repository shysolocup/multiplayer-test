...
public static class NodeE
{
	...
	#region GetDescendantOfType

	/// <summary>
    /// <para>Gets the first descendant of the <see cref="Node"/> matching the given generic type or class.</para>
	/// <para><code>
	/// var label = container.GetDescendantOfType&lt;Label&gt;(); // get the label from the container
	/// </code></para>
    /// </summary>
	public static T GetDescendantOfType<T>(this Node self, int layers = 100) where T : Node
	{
		Node current = self;
		for (int i = 0; i < layers; i++)
		{
